    ...
    def update(self, memory, update_time):
        ...
        if use_cv > 0 and update_time < phi_max_bases:
            ...
            if len(self.control_variate.bases) > 1:
                phi.load_state_dict(self.control_variate.bases[-1].state_dict())
            # optimizer and fixed step size
            phi_optim = torch.optim.Adam(phi.parameters(), lr=phi_lr, betas=phi_betas)

            # get current phi_values, and phi_grad_actions
            old_phi_values, old_phi_grad_actions = self.control_variate.get_value(old_states, old_actions)
        ...
        for _ in range(self.K_epochs):
            ...
            surr = ratios.unsqueeze(-1) * (
                    ll_grad_mu * (advantages.unsqueeze(-1) - use_cv * phi_values) + use_cv * phi_grad_actions)
...
def main():
    ...
    for i_episode in range(1, max_episodes + 1):
        ...
        if i_episode % log_interval == 0:
            ...
            utils.write_to_file_data('reward_records_gbphi.txt', running_reward)
